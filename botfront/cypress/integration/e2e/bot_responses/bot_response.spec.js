describe('Bot responses', () => {
    ...
    it('Create and remove response using the response editor', () => {
    ...
    it('Edit a response using the response editor', () => {
        addTextResponse('test_A', 'response content')
        cy.dataCy('edit-response-0').click()
        cy.dataCy('response-name-input').click().find('input').type('{backspace}B')
        cy.dataCy('bot-response-input').click({ force: true }).find('textarea').clear().type('new response').blur()
        cy.escapeModal()
    ...
    it('Allow the response to be edited in another language', () => {
        addTextResponse('test_A', 'response content')
        cy.dataCy('template-intent').contains('utter_test_A').should('exist')
        ...
        cy.dataCy('edit-response-0').click()
        cy.dataCy('bot-response-input').click().find('textarea').clear().type('new response').blur()
        cy.escapeModal()
        cy.dataCy('template-intent').contains('utter_test_A').should('exist')
        cy.dataCy('response-text').contains('new response').should('exist')
    ...
    it('Do not allow duplicate names when creating a response', () => {
    ...
    it('Do not allow duplicate names when editing a response', () => {
        //Add the first response
        addTextResponse('test_A', 'response content')
    ...
    it('Require response names to start with utter_ and not contain spaces', () => {
    ...
    it('Create and edit response using the response editor', () => {
        cy.visit('/project/bf/responses')
        ...
        cy.get('.-filters').find('input').first().click().type('A')
        ...
        cy.dataCy('response-name-input').click().find('input').clear().type('utter_test_D').blur()
        cy.dataCy('bot-response-input').find('textarea').clear().type('edited content').blur()
        cy.escapeModal()
        cy.dataCy('template-intent').should('have.length', 1)
        cy.dataCy('template-intent').contains('utter_test_A').should('not.exist')
        cy.get('.-filters').find('input').first().click().clear()
        cy.dataCy('template-intent').should('have.length', 3)
        cy.dataCy('template-intent').contains('utter_test_D').parents('.rt-tr').find('[data-cy=response-text]').should('contain.text', 'edited content')
    })

    it('Edit a response with the response editor in the visual story editor', () => {
        ...
        cy.get('textarea').type('- action: utter_test_A').blur()
        cy.wait(700)
        ...
        cy.dataCy('response-editor').find('[data-cy=bot-response-input]').find('textarea').type('{backspace}{backspace}edited by response editor').blur()
        cy.dataCy('metadata-tab').click()
    ...
    it('Open other stories that use a response a story with that response', () => {
        cy.visit('/project/bf/responses')
        ...
        cy.dataCy('toggle-yaml').click()
        cy.get('.ace_content').click({ force: true })
        cy.get('textarea').type('- action: utter_test').blur()
