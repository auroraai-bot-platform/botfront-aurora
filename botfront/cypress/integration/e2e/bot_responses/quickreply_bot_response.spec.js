describe(' Quick reply bot responses', () => {
    beforeEach(() => {
        cy.login()
        cy.deleteProject('bf')
    ...
    it('Create a quick reply response using the response editor', () => {
    ...
    it('Edit a quick reply response using the response editor', () => {
        cy.createResponseFromResponseMenu('quickreply', 'test_A')
        cy.dataCy('bot-response-input').click().find('textarea').type('response text')
        ...
        cy.dataCy('edit-response-0').click()
        cy.dataCy('bot-response-input').click().find('textarea').clear().type('updated text').blur()
        //cy.wait(500)
    ...
    it('Create quick reply variations', () => {
        cy.createResponseFromResponseMenu('quickreply', 'test_A')

        cy.dataCy('bot-response-input').click().find('textarea').type('response text deleted').blur()
        cy.dataCy('button_title').click({ force: true })
        ...
        cy.dataCy('bot-response-input').last().click().find('textarea').type('response text B').blur()
    ...
    it('Turn a quick reply into a textWithButtons response', () => {
        cy.createNLUModelProgramatically('bf', '', 'fr')
        cy.visit('/project/bf/dialogue')
        cy.createStoryGroup()
        cy.createFragmentInGroup()
        ...
        cy.setQuickReplyContent('purple', 'purple', 'purple', 0, 0)
        cy.setQuickReplyContent('test', 'test', 'test', 0, 1)
        cy.escapeModal()
        cy.dataCy('language-selector').click()
        cy.dataCy('language-selector').find('div').contains('French').click()
        cy.dataCy('story-title').should('have.value', 'Groupo (1)')
        cy.dataCy('edit-responses').click({ force: true })
        cy.setQuickReplyContent('french', 'french', 'french', 0, 0)
        cy.escapeModal()
