describe('Bot response variations', () => {
    beforeEach(() => {
        cy.login()
        cy.deleteProject('bf')
    ...
    const addVariation = (text) => {
        ...
        cy.dataCy('bot-response-input').last().click().find('textarea').type(text)
    }

    it('Add and remove variations while creating a response', () => {
        cy.createResponseFromResponseMenu('text', 'test')
        cy.dataCy('bot-response-input').click().find('textarea').type('A')
        addVariation('remove B')
        addVariation('C{enter}')
        cy.dataCy('icon-trash').eq(1).click()
        cy.dataCy('bot-response-input', 'A').should('exist')
        cy.dataCy('bot-response-input', 'C').should('exist')
        cy.dataCy('bot-response-input', 'remove B').should('not.exist')
        cy.escapeModal()
        cy.dataCy('response-text').find('div').contains('A').should('exist')
        cy.dataCy('response-text').find('div').contains('C').should('exist')
        cy.dataCy('response-text').find('div').contains('remove B').should('not.exist')
    })

    it('Add and remove variations while editing a response', () => {
        cy.createResponseFromResponseMenu('text', 'test')
        ...
        cy.dataCy('bot-response-input').eq(0).click().find('textarea').clear().type('edited A')
        cy.dataCy('bot-response-input').eq(1).click().find('textarea').clear().type('edited deleted')
        cy.dataCy('bot-response-input').eq(2).click().find('textarea').clear().type('edited B{enter}')
        cy.dataCy('icon-trash').eq(1).click({ force: true })
    ...
    it('Show first variation in the visual editor', () => {
        ...
        cy.dataCy('bot-response-input').last().find('textarea').clear().type('bye')
        cy.escapeModal()
