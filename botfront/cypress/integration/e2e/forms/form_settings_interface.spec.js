...
describe('Main form editor interface usage', () => {
    beforeEach(() => {
        cy.login()
        cy.deleteProject('bf')
    ...
    it('Change the response type via the dropdown', () => {       
        ...
        // edit response
        cy.dataCy('slot-node-wrapper-slot1').find('[data-cy=bot-response-input]').find('textarea').type('categorical slot').blur()
        cy.reload()
        cy.get('.loader').should('not.exist')
        cy.selectForm(newFormName)
    ...
    it('Set all slot types', () => {       
        ...
        cy.dataCy('extraction-source-dropdown').children().find('div.item').last().click()
        ...
        cy.dataCy('extraction-source-dropdown').last().find('span').contains('intent').click()
        cy.dataCy('intent-condition-dropdown').last().click()
        cy.dataCy('intent-condition-dropdown').last().find('span').contains('NOT').click()
        ...
        cy.dataCy('category-value-dropdown').last().find('span').contains('blue').click()
        cy.dataCy('add-condition').click()
        cy.dataCy('extraction-source-dropdown').should('have.length', 3)
        cy.dataCy('extraction-source-dropdown').last().click()
        cy.dataCy('extraction-source-dropdown').last().find('span').contains('from the user message').click()
