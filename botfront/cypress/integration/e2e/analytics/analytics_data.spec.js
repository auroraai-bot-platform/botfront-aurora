...
describe('Analytics data', () => {
    ...
    it('Display all intents in the table', () => {
        cy.addCustomConversation('bf', 'top10intents', {
            events: top10IntentEvents,
        })
        cy.dataCy('analytics-card').should('have.length', 7)

        // check that the limit filter does not affect to the table
        cy.dataCy('analytics-card').eq(2).find('rect').filter('[stroke="rgb(29, 111, 168)"]').should('have.length', 10)
        ...
        cy.dataCy('analytics-card').eq(2).find('rect').filter('[stroke="rgb(29, 111, 168)"]').should('have.length', 5)
        ...
        cy.dataCy('analytics-card').eq(2).find('rect').filter('[stroke="rgb(29, 111, 168)"]').should('have.length', 11)
    })

    it('Apply conversation over time filters selected in the ellipsis menu', () => {
        ...
        cy.dataCy('settings-portal-input').click()
        cy.dataCy('settings-portal-input').find('input').clear().type('5`{enter}{esc}')
        cy.dataCy('analytics-card').first().find('.rt-tbody').find('.rt-tr-group').eq(6).find('.rt-td.number-column').first().should('have.text', '0')
        ...
        cy.dataCy('analytics-card').first().find('.rt-tbody').find('.rt-tr-group').eq(6).find('.rt-td.number-column').first().should('have.text', '1')
    })

    it('Display a trigger intent card and filter by intent type', () => {
        ...
        cy.dataCy('analytics-card').first().find('rect').filter('[stroke="rgb(29, 111, 168)"]').should('have.length', 1)
        cy.dataCy('table-chart-button').first().click()
        ...
        cy.dataCy('analytics-card').eq(3).find('rect').filter('[stroke="rgb(29, 111, 168)"]').should('have.length', 1)
        ...
        cy.dataCy('analytics-card').eq(1).find('.rt-tbody').find('.rt-tr-group').eq(6).find('.rt-td.number-column').first().should('have.text', '2')
        ...
        cy.dataCy('analytics-card').eq(1).find('.rt-tbody').find('.rt-tr-group').eq(6).find('.rt-td.number-column').first().should('have.text', '1')
