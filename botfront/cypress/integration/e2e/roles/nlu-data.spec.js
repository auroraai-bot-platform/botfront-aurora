describe('NLU-data:r restricted permissions', () => {
    ...
    it('should not show forbidden sections', () => {
        cy.get('.ui.pointing.secondary.menu').should('exist').should('contain', 'Examples').should('not.contain', 'Chit Chat').should('not.contain', 'Insert Many')
        cy.dataCy('nlu-menu-evaluation').should('not.exist')
        cy.dataCy('nlu-menu-settings').click()
        cy.get('.ace_editor').find('textarea').click({ force: true }).type('TYPING BUT FAILING', { force: true })
        cy.get('.ace_editor').should('contain.text', '- name:').should('not.contain.text', 'TYPING BUT FAILING')
        cy.dataCy('save-button').should('not.exist')
        cy.get('.ui.pointing.secondary.menu').should('exist').should('not.contain', 'Delete')
    })

    it('should not show nlu example editor in playground or train button', () => {
        cy.dataCy('example-text-editor-input').should('exist').click().type('hello')
