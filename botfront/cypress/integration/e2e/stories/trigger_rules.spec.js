...
describe('Smart story trigger rules', () => {
    beforeEach(() => {
        cy.login()
        cy.deleteProject('bf')
    ...
    it('edit, save and query the trigger rules', () => {
        ...
        cy.dataCy('payload-text-input').first().click().find('input').type('test payload')
        cy.dataCy('toggle-website-visits').first().click()
        cy.dataCy('website-visits-input').first().click().find('input').type('3')

        // edit the second ruleset
        cy.dataCy('toggle-page-visits').last().click()
        cy.dataCy('page-visits-input').last().click().find('input').type('4')
        cy.dataCy('toggle-query-string').last().click()
        cy.dataCy('query-string-field').last().find('input').first().click().type('name')
        cy.dataCy('query-string-field').last().find('input').eq(1).click().type('value')
        ...
        cy.dataCy('event-selector').last().find('.item').first().click()
    ...
    it('delete trigger rules with the delete button', () => {
        cy.browseToStory('Get started')
        cy.dataCy('edit-trigger-rules').click()
    ...
    it('clear disabled fields on close', () => {
        cy.browseToStory('Get started')
        cy.get('.utterances-container').first().findCy('icon-trash').click({ force: true })
        cy.dataCy('edit-trigger-rules').click()
        ...
        cy.dataCy('payload-text-input').click().find('input').type('test payload')
        cy.dataCy('toggle-website-visits').click()
        cy.dataCy('website-visits-input').click().find('input').type('3')
    ...
    it('do not allow a destination story to have rules', () => {
        cy.createStoryGroup()
    ...
    it('do not allow linking to a story with rules', () => {
        cy.createStoryGroup()
        cy.createFragmentInGroup({ fragmentName: 'like wooh' })
        cy.createFragmentInGroup({ fragmentName: 'like woah' })
        cy.dataCy('edit-trigger-rules').click()
    ...
    it('trigger a story with the rules payload', () => {
