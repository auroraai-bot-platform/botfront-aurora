...
describe('Bot responses copy between languages', () => {
    afterEach(() => {
        cy.deleteProject('bf')
    })
    ...
    it('allow copy from another language with a text response', () => {
        cy.browseToStory('Groupo (1)')
        cy.dataCy('add-bot-line').click({ force: true })
        cy.dataCy('from-text-template').click({ force: true })
        cy.dataCy('bot-response-input').find('textarea').clear().type('test response')
    ...
    it('allow copy from another language with a image response', () => {
        cy.browseToStory('Groupo (1)')
        cy.dataCy('add-bot-line').click({ force: true })
        ...
        cy.dataCy('language-selector').click()
        cy.contains('German').click()
        cy.dataCy('bot-response-input').find('img').should('have.attr', 'src').and('equal', '/images/image-temp.svg')
        cy.dataCy('import-from-lang').click({ force: true })
        cy.dataCy('import-from-lang').contains('English').click()
        cy.get('[data-cy=image-container] img').should('have.attr', 'src', imageUrlA)
    })

    it('allow copy from another language with a button response', () => {
        cy.dataCy('language-selector').click()
        cy.dataCy('add-bot-line').click({ force: true })
        cy.dataCy('from-qr-template').click({ force: true })
        cy.dataCy('bot-response-input').find('textarea').type('buttons test').blur()
    ...
    it('allow copy from another language with a carousel response', () => {
        cy.dataCy('language-selector').click()
        cy.dataCy('add-bot-line').click({ force: true })
        ...
        cy.dataCy('import-from-lang').click({ force: true })
        cy.dataCy('import-from-lang').contains('English').click()
        cy.get('.carousel-slide img').should('have.attr', 'src').and('equal', imageUrlA)
