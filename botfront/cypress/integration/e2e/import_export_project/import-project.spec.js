...
describe('Import Botfront project', () => {
    beforeEach(() => {
        cy.login()
        cy.deleteProject('bf')
    ...
    it('mark as error a file that is not json/yaml/zip/yml/md', () => {
    ...
    it('import a zip', () => {
        ...
        + '1 test will not be added as Korean is not a project language:korean test case')
            ...
            cy.dataCy('language-selector').click().find('div').contains('French').click()
            cy.dataCy('nlu-menu-training-data').click()
            cy.get('.row').should('have.length', 19)
            cy.dataCy('nlu-menu-settings').click()
            cy.get('.ace_content .ace_text-layer').should('contain.text', '- name: WhitespaceTokenizer')
     
            cy.dataCy('language-selector').click().find('div').contains('Russian').click()
            cy.dataCy('nlu-menu-training-data').click()
            cy.get('.row').should('have.length', 1)
            cy.dataCy('nlu-menu-settings').click()
            cy.get('.ace_content').should('contain.text', '- name: TestDummyLineRu')
            cy.dataCy('language-selector').click().find('div').contains('German').click()
            cy.dataCy('nlu-menu-training-data').click()
            cy.get('.row').should('have.length', 1)
            cy.dataCy('nlu-menu-settings').click()
    ...
    it('import a file with wipe project', () => {
        cy.task('zipFolder', 'test-project-cypress').then(async (b64zip) => {
        // import a project so there is something to wipe
            ...
            cy.visit('/project/bf/settings/endpoints')
            cy.get('.ace_content').should('contain.text', 'rasa_addons.core.nlg.GraphQLNaturalLanguageGenerator')
            cy.visit('/project/bf/settings/credentials')
