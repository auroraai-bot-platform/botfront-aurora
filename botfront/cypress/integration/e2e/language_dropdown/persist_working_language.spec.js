describe('Redux working language', () => {
    beforeEach(() => {
        cy.login()
        cy.deleteProject('bf')
    ...
    it('persist the working language', () => {
        ...
        */
        ...
        checkLanguage('French')
        cy.dataCy('nlu-sidebar-link').click({ force: true })
        checkLanguage('French')
        cy.dataCy('intent-label').should('not.exist')

        // switch the language back to english in nlu
        selectLanguage('English')
        checkLanguage('English')
        cy.dataCy('intent-label').should('exist')
