...
export default {
    ...
    Mutation: {
        async updateExamples(_, args, context) {
            ...
            insertChanges('dev', args.projectId, context.user.emails[0].address, 'updated examples', args['examples'][0]['_id'], 'none', JSON.stringify(response[0]));
        ...
        async insertExamples(_, { autoAssignCanonical, overwriteOnSameText, ...args }, context) {
            ...
            insertChanges('dev', args.projectId, context.user.emails[0].address, 'inserted examples', response[0]['_id'], 'none', JSON.stringify(response[0]));
        ...
        async deleteExamples(_, args, context) {
            ...
            insertChanges('dev', args.projectId, context.user.emails[0].address, 'deleted examples', response[0], 'none', 'none');
        ...
        async switchCanonical(_, args, context) {
            ...
            insertChanges('dev', args.projectId, context.user.emails[0].address, 'deleted examples', args['examples'][0]['_id'], JSON.stringify(args.example), JSON.stringify(response[0]));
            auditLog('deleted examples', {
                user: context.user,
                type: 'deleted',
                projectId: args.projectId,
